...
// 修订日期:		2021年07月12日
// 修订历史:
// 2019年08月25日	V1.0		创建文件,编写视频驱动模块(Driver_Video)
// 2019年09月10日	V1.1		增加视频彩条功能,支持视频1080p@60Hz输出
// 2019年09月15日	V1.2		增加视频像素坐标输出
// 2020年03月14日	V1.3		删减彩条输出,将RGB数据输出移至其他模块,此版本不再支持
// 2020年07月23日	V1.4		增加视频多格式输出及选择,支持480*272p、1920*1080p等诸多视频格式输出
// 2021年07月12日	V1.5		更名为Video_Generator_Interface
...
module Video_Generator_Interface(
  ...
  begin
    ...
    else if(hsync_cnt >= H_FP + H_SYNC + H_BP - 1)
    begin
      set_x_o <= hsync_cnt - (H_FP + H_SYNC + H_BP - 1);//当行信号有效信号,开始获取X坐标
    end
    else
    begin
      set_x_o <= set_x_o;
  ...
  begin
    ...
    else if(vsync_cnt >= V_FP + V_SYNC + V_BP - 1)
    begin
      set_y_o <= vsync_cnt - (V_FP + V_SYNC + V_BP - 1);//当场信号有效信号,开始获取当前Y坐标
    end
    else
    begin
      set_y_o <= set_y_o;
  ...
  begin
    if(i_rstn == 1'b0)
      rgb_vsync_o <= 0;
    else if(hsync_cnt == H_FP-1 && vsync_cnt == V_FP - 1)
      rgb_vsync_o <= VS_POL;
    else if(hsync_cnt == H_FP-1 && vsync_cnt == V_FP + V_SYNC - 1)
      rgb_vsync_o <= ~VS_POL;
    else
  ...
  begin
    if(i_rstn == 1'b0)
      v_de <= 1'b0;
    else if(hsync_cnt == H_FP - 1 && vsync_cnt == V_FP + V_SYNC + V_BP - 1)
      v_de <= 1'b1;
    else if(hsync_cnt == H_FP - 1 && vsync_cnt == V_TOTAL - 1)
      v_de <= 1'b0;
    else
      v_de <= v_de;
  end
